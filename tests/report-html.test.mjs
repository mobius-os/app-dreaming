...
// ---------------------------------------------------------------------------
// answersLanded: the re-read durability gate. window.mobius.storage.set()
// reports {synced:true} even for a server-FATALLY-rejected write (it dead-letters
// the poison op and reconciles the mirror back to the server's value), so the
// question card re-reads question-answers/<date>.json and only flips to "Saved"
// when the stored payload reflects what it wrote. These lock that the gate
// accepts a true write and REJECTS the false-saved cases (missing/stale/rejected).
// ---------------------------------------------------------------------------

test('answersLanded accepts a re-read that reflects every written answer', async () => {
  const { answersLanded } = await bundle()
  const written = {
    report_date: '2026-06-22',
    answers: { 'Which apps?': 'Notes', 'How long?': 'Short' },
  }
  // The server stored exactly what we wrote (it may add fields like answered_at —
  // that doesn't matter; only the answers + date we wrote must be reflected).
  const stored = { report_date: '2026-06-22', answered_at: 'x', answers: { 'Which apps?': 'Notes', 'How long?': 'Short' } }
  assert.equal(answersLanded(stored, written), true)
})

test('answersLanded REJECTS a stale re-read missing the answers (fatal-reject dead-letter)', async () => {
  const { answersLanded } = await bundle()
  const written = { report_date: '2026-06-22', answers: { 'Which apps?': 'Notes' } }
  // A 413/400/415/403 reject dead-letters the op and the mirror reconciles to
  // the server's value — here, no prior record at all. set() still said {synced}.
  assert.equal(answersLanded(null, written), false)
  assert.equal(answersLanded({ notFound: true }, written), false)
  // Or the server holds an OLDER answer set for the date — not what we wrote.
  assert.equal(
    answersLanded({ report_date: '2026-06-22', answers: { 'Which apps?': 'Habits' } }, written),
    false,
  )
  // Or a partial write where one answer is missing.
  assert.equal(
    answersLanded({ report_date: '2026-06-22', answers: {} }, written),
    false,
  )
})

test('answersLanded REJECTS a payload for the wrong report date', async () => {
  const { answersLanded } = await bundle()
  const written = { report_date: '2026-06-22', answers: { Q: 'A' } }
  assert.equal(
    answersLanded({ report_date: '2026-06-21', answers: { Q: 'A' } }, written),
    false,
  )
})

test('answersLanded never throws on malformed stored input', async () => {
  const { answersLanded } = await bundle()
  const written = { report_date: '2026-06-22', answers: { Q: 'A' } }
  assert.equal(answersLanded(undefined, written), false)
  assert.equal(answersLanded('nope', written), false)
  assert.equal(answersLanded({ report_date: '2026-06-22' }, written), false)
  assert.equal(answersLanded({ report_date: '2026-06-22', answers: 'nope' }, written), false)
})
